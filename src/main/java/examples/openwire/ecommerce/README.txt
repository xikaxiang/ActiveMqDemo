...
the retailer who places the orders, the vendor（卖家） who assemples the computers,
and two suppliers--one for hard drives and another for monitors.
客户下等订单  卖家  供应商（2家）
The retailer（零售商） sends a message to the vendor's queue and awaits a reply.
...
throw an exception simulating（模拟） a database error, triggering a rollback.
Each supplier receives the order, checks inventory（库存） and replies to the
